...
前提：Windows/macOS 已安装 Docker Desktop，Linux 已安装 Docker Engine，并确保 Docker 正在运行。首次启动需要构建 Docker 镜像，通常需要 3-8 分钟。
...
- `axis start` 支持 Windows 和 macOS；Linux 需要自行安装并启动 Docker Engine。`npm run dev` 和单服务启动不受此限制。
