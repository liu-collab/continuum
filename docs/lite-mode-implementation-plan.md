...
- [ ] 实现 lite 记忆模型配置读取：managed config/secrets + `MEMORY_LLM_*` 环境变量
- [ ] 实现 `MemoryOrchestrator.prepareContext()` 和规则触发器
- [ ] 实现记忆模型内部 `memory_search` function call
- [ ] 实现记忆模型内部 `memory_get` function call
- [ ] 实现进程内 Promise 串行写队列
- [ ] 实现 lite runtime HTTP 入口：`prepare-context` / `after-response` / `healthz`
- [ ] 单元测试
- [ ] Lite runtime E2E：prepare-context 命中/不命中、after-response 写入、规则降级、trace 生成

### Phase 1（2-3 天）：适配器 + CLI

- [ ] `axis claude install` 命令：写入 hook 配置，指向 lite runtime HTTP
- [ ] `axis codex use` 命令：配置 Codex 适配器，指向 lite runtime HTTP
- [ ] `axis memory-model configure` 命令：写入记忆模型 managed config/secrets
- [ ] 宿主安装时检测记忆模型配置，未配置则提示可跳过
- [ ] `axis start` / `axis start --full` 命令
- [ ] 宿主适配器自动检测 lite runtime 是否存在
- [ ] 宿主适配 E2E：Claude/Codex 适配器自动拉起 lite runtime、自动注入、不通过 MCP 主动调用

### Phase 2（2-3 天）：写入链路适配

- [ ] writeback-engine 增加 lite 写入路径
- [ ] 多轮上下文传递给 LLM
- [ ] 硬校验逻辑实现
- [ ] 写入 E2E：多轮上下文提取、敏感信息过滤、并发写入串行化、outbox 重试

### Phase 3（1-2 天）：页面 + 迁移

- [ ] Lite 页面（记忆列表、trace 查看、记忆模型配置）
- [ ] 页面 Agent 设置支持记忆模型健康检查和降级状态展示
- [ ] `axis migrate --to full` 命令
- [ ] 智能升级提示
- [ ] 迁移 E2E：启动 full 时提示迁移、选择 Y 后导入、选择 N 后不重复提示、失败后可继续 full 空数据
