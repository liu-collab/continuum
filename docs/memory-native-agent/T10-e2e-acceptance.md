...
- 已补并跑通 `mcp-call.e2e.test.ts`、`mna-restart.e2e.test.ts`
- 已补并跑通 `services/visualization/tests/agent-e2e/agent-recovery.spec.ts`
- 已补并跑通 `services/visualization/tests/agent-e2e/agent-extra-ui.spec.ts` 中的 locale、provider、abort、文件树、replay gap、已有会话切换、深链恢复、session error
- 已补并跑通 `services/visualization/tests/agent-e2e/agent-refresh.spec.ts`、`agent-shell-deny.spec.ts`
- 已补并跑通 `services/memory-native-agent/tests/e2e/record-replay-provider.e2e.test.ts`
- 已补并跑通 `services/visualization/tests/agent-e2e/agent-record-replay.spec.ts`
...
├── tests/
│   ├── e2e/
│   │   ├── setup.ts                      # 已完成：测试期装配 mna + runtime + storage + provider stub / record-replay
...
│   │   └── record-replay-provider.e2e.test.ts # 已完成
│   └── fixtures/
│       ├── model-record-replay/
...
Playwright 跑在 visualization 启动的环境下，mna 用真实后端进程（而非 mock）；默认回归可继续使用 stub provider，`record-replay` 基线用例单独通过环境变量切到 replay 模式。
...
- 自动化层面：
  - `services/memory-native-agent` 已通过相关后端 e2e 与 HTTP 测试：`mna restart`、session routes
  - 已落地并跑通真实后端 e2e：`happy path / runtime down / storage down / memory_mode switch / governance roundtrip / mcp_call / mna restart / record-replay provider`
  - `services/visualization` 已跑通真实 Playwright：`mna restart` 恢复、locale、provider、顶部刷新、abort、文件树、shell 黑名单前端反馈、replay gap、已有会话切换、深链恢复、session error、record-replay provider`
- 人工层面：已整理成 `acceptance-checklist.md`
- 未完成项仍然存在：整套服务交付级人工复核仍待补
