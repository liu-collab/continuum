...
│   │   ├── degrade-runtime-down.e2e.test.ts
│   │   ├── degrade-storage-down.e2e.test.ts   # 未完成
│   │   ├── memory-mode-switch.e2e.test.ts
│   │   └── governance-roundtrip.e2e.test.ts   # 未完成
│   └── fixtures/
│       ├── model-record-replay/
...
└── agent-ui.spec.ts              # Playwright 覆盖前端用户流
...
- 自动化层面：
  ...
  - 已落地并跑通 3 条真实后端 e2e：`happy path / runtime down / memory_mode switch`
- 人工层面：已整理成 `acceptance-checklist.md`
- 未完成项仍然存在：`storage down`、治理闭环、Playwright UI e2e
