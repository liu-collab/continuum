...
- 技术选型：首版实际采用 `services/visualization/src/app/agent/_i18n/provider.tsx` 的局部轻量 provider，不引入全局 `next-intl`
...
- 已完成：
  ...
  - `visualization` agent 页面已补 `_i18n/{zh-CN,en-US}/common.json`、局部 i18n provider、顶部 locale 切换和 confirm 文案本地化
- 待后续任务继续收口：
