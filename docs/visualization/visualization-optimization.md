...
| **P1** | 优化十一（治理配置入口） | 用户无法控制自动治理，功能缺失 |
| **P1** | 优化十三（删除确认） | 破坏性操作无二次确认 |
| **P1** | 优化十七（修复 /agent 空页） | 导航栏第一个入口不可用 |
| **P2** | 优化二（合并 i18n 系统） | 架构一致性 |
| **P2** | 优化五（消除重复） | 代码维护性 |
| **P2** | 优化六（Link 替代 a 标签） | 用户体验 |
| **P2** | 优化七（阈值配置化） | 运维效率 |
| **P2** | 优化十二（跨页追溯链路） | 数据血缘可追溯 |
| **P2** | 优化十四（操作反馈） | 治理操作后无状态反馈 |
| **P2** | 优化十五（筛选 loading 过渡） | 无加载态导致感知卡顿 |
| **P2** | 优化十六（Evidence 格式化） | 原始 JSON 不可读 |
...
DESIGN-SYSTEM.md（Apple 风格稿）定义了完整的颜色、字体、间距、圆角、阴影体系。以下为全局 + 各页面的偏差审计结果。
