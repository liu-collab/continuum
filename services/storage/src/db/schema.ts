...
const privateSchema = pgSchema(
  process.env.STORAGE_SCHEMA_PRIVATE ?? "storage_private",
);
const sharedSchema = pgSchema(
  process.env.STORAGE_SCHEMA_SHARED ?? "storage_shared_v1",
);
...
export const memoryRecords = privateSchema.table(
  "memory_records",
  {
...
export const memoryRecordVersions = privateSchema.table(
...
export const memoryWriteJobs = privateSchema.table(
  "memory_write_jobs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
...
export const memoryConflicts = privateSchema.table(
  "memory_conflicts",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    workspaceId: uuid("workspace_id").notNull(),
...
export const memoryGovernanceActions = privateSchema.table(
...
export const memoryReadModel = sharedSchema.table(
...
export const memoryReadModelRefreshJobs = privateSchema.table(
