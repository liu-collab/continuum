...
export default function MemoryDetailError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  const { t } = useAppI18n();
  ...
  return (
    <PageError
      ...
      heading={t("memories.detail.kicker")}
      subtitle={t("common.retryLater")}
      testId="memory-detail-error-state"
    />
  );
}
