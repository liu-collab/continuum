...
| #8 | 已完成 | 已补治理提案量、复核通过率、执行成功率、软删除率、重试率和治理后召回命中率看板指标 |
...
| 新增观测指标 | 已完成 | 已补提案量、复核通过率、执行成功率、软删除率、重试率，并提供治理后召回命中率近窗估算口径 |
...
- 其中“治理后召回命中率”当前按 visualization 侧近窗运行轨迹做估算：统计最近已执行治理动作后，后续召回再次命中相关目标记录的占比；后续若 runtime 补充更稳定的执行关联字段，再切到服务端正式口径
