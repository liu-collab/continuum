...
| `recall_accuracy_proxy`（召回准确率代理值） | > 80% | 3 / 3 = 100% | 通过 |
| `low_quality_intercept_rate`（低质量拦截率） | > 80% | 1 / 1 = 100% | 通过 |
| `relation_discovery_accuracy`（关联发现准确率） | > 80% | 1 / 2 = 50% | 未达标 |
| `recommendation_acceptance_proxy`（推荐采纳率代理值） | > 30% | 1 / 2 = 50% | 通过 |
| `knowledge_extraction_accuracy`（知识提取准确率） | > 85% | 2 / 2 = 100% | 通过 |
| `governance_correctness_proxy`（治理正确率代理值） | > 85% | 2 / 2 = 100% | 通过 |
...
> 本轮已按剩余验收项扩样本，重点补了召回 / 治理 / 关联 / 推荐 / 知识提取 5 类样本统计；当前这里展示的是最新一轮结果。
...
|---|---|---|---|---|
| 召回准确率 | 人工标注样本中，最终召回是否正确帮助当前问题 | > 80% | 离线样本 3 / 3 = 100%，人工值未采集 | 待补人工标注 |
| 治理正确率 | 治理动作是否符合预期，没有误归档、误合并、误删除 | > 85% | 离线样本 2 / 2 = 100%，人工值未采集 | 待补人工标注 |
| 低质量拦截率 | 质量评估对低质量候选的拦截效果 | > 80% | 离线代理值 100%，人工值未采集 | 待补人工标注 |
| 优先级调整后命中率提升 | 效果反馈回写重要度后，对后续命中率的提升 | > 20% | 未采集 | 待近窗对比 |
| 意图理解准确率 | `intent analyzer`（意图分析器）对是否需要记忆、作用域、类型的判断准确率 | > 90% | 离线样本 2 / 2 = 100%，人工值未采集 | 待补人工标注 |
| 关联发现准确率 | `relation discoverer`（关系发现器）输出关系是否成立 | > 80% | 离线样本 1 / 2 = 50%，人工值未采集 | 待补人工标注 |
| 推荐采纳率 | 主动推荐是否被宿主/用户实际采用 | > 30% | 离线代理值 1 / 2 = 50%，线上值未采集 | 待产品埋点 |
| 知识提取准确率 | 演化提取出的长期知识是否正确、可复用 | > 85% | 离线样本 2 / 2 = 100%，人工值未采集 | 待补人工标注 |
