...
describe("tierMemoryInjection", () => {
  ...
  it("does not create memory_summary when only high and medium records remain", () => {
    const result = tierMemoryInjection({
      phase: "before_response",
      injection_reason: "history reference",
      memory_summary: "偏好与任务状态",
      memory_records: [
        {
          id: "pref-1",
          memory_type: "fact_preference",
    ...
    expect(result.high.map((record) => record.id)).toEqual(["pref-1"]);
    expect(result.medium.map((record) => record.id)).toEqual(["task-1"]);
    expect(result.summary_records).toEqual([]);
    expect(result.summary).toBeNull();
  });

  it("keeps summary-only memory when there are no retained records", () => {
    const result = tierMemoryInjection({
      phase: "before_response",
      injection_reason: "history reference",
      memory_summary: "只保留摘要",
      memory_records: [],
    });

    expect(result.summary_records).toEqual([]);
    expect(result.summary).toBe("只保留摘要");
  });
});
