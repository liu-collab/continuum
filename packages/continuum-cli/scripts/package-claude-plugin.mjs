...
const pluginStageDir = path.join(releaseDir, "continuum-claude-plugin");
...
async function rewritePluginArtifacts(stageDir, packageName, version) {
  const runtimeCommand = `npx -y -p ${packageName}@${version} continuum runtime`;
  const mcpCommand = `npx -y -p ${packageName}@${version} continuum mcp-server`;
...
async function main() {
  ...
  const zipPath = path.join(releaseDir, `continuum-claude-plugin-v${packageJson.version}.zip`);
