...
export function buildWindowsLegacyContinuumCleanupScript() {
  const pattern =
    "vendor/(storage/dist/src/server\\.js|storage/dist/src/worker\\.js|runtime/dist/src/index\\.js|visualization/standalone/server\\.js|memory-native-agent/bin/mna-server\\.mjs)|continuum.*embeddings|local-embedding-service\\.js";
  const repoServicesRoot = process.env.CONTINUUM_REPO_ROOT
    ? `${normalizeWindowsPathForMatch(process.env.CONTINUUM_REPO_ROOT)}/services/`
    : "";
